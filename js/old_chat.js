    ...
    async handleMessage(prompt, hasPdfFile, hasAttachments) {
        try {
            ...
            if (prompt) {
                ...
                this.messageBuilder.messages.push({
                    role: "user", 
                    content: prompt
                });
            }
            ...
            if (selectedText) {
                userMessage.text += (userMessage.text ? ' ' : '') + selectedText; // 添加选中的文本
                this.messageBuilder.messages.push({
                    role: "user", 
            ...
            if (hasPdfFile && window.currentPdfFile) {
                ...
                await this.messageBuilder.parsingPdf(window.currentPdfFile);
            }
            ...
            if (hasAttachments && Array.isArray(this.attachments)) {
                for (const attachment of this.attachments) {
                    if (attachment.url) {
                        ...
                        await this.messageBuilder.parsingImage(attachment.url);
                    }
                }
            }
            ...
            this.messageBuilder.messages.push({ "role": "assistant", "content": aiResponse })
