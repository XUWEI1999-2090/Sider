class ChatManager {
                ...
                chrome.storage.local.get(
                    ...
                    (result) => {
                        ...
                        if (
                            ...
                            if (
                                currentId &&
                                this.getConversationById(currentId)
                            ) {
    ...
    loadCurrentConversation() {
        const conversation = this.getConversationById(
            this.currentConversationId,
        );
        if (conversation) {
            // 更新存储中的当前会话ID
            ...
            if (this.messageContainer) {
                ...
                if (conversation.messages && conversation.messages.length > 0) {
                    console.log(
                        `Rendering ${conversation.messages.length} messages for conversation ${conversation.id}`,
                    );
            ...
            console.warn(
                "Failed to load conversation with ID:",
                this.currentConversationId,
            );
            // 如果找不到当前对话，创建一个新的
            this.createNewConversation();
    ...
    switchConversation(id) {
        ...
        console.log(
            `Loading conversation with ID ${id} and ${conversation.messages ? conversation.messages.length : 0} messages`,
        );
    ...
    updateConversationTitle(id, firstMessage) {
        ...
        if (conversation && conversation.title === "新对话" && firstMessage) {
            ...
            conversation.title =
                firstMessage.length > maxTitleLength
    ...
    async handleMessageSubmit() {
        ...
        const hasPdfFile =
            window.currentPdfFile !== undefined &&
        ...
        // 确定是否需要切换到多模态模型
        if (
            conversation.modelType === "text" &&
            (hasAttachments || hasPdfFile)
        ) {
            ...
            if (hasPdfFile || hasAttachments) {
                ...
                try {
                    ...
                    if (hasPdfFile && window.currentPdfFile) {
                        ...
                        // 先将整个PDF作为一个对象发送
                        const pdfBase64 = await this.fileToBase64(
                    ...
                    if (hasAttachments && this.attachments) {
                        ...
                        for (const attachment of this.attachments) {
                            if (
                                ...
                                const base64 =
                                    await this.fileToBase64(attachment);
                    ...
                    const res = await fetch(
                        "https://openrouter.ai/api/v1/chat/completions",
                        {
                            ...
                            headers: {
                                Authorization:
                                    "Bearer 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",
                ...
                const options = isMultimodal
                    ? {
                          ...
                          headers: {
                              Authorization:
                                  "Bearer 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",
                              "Content-Type": "application/json",
                    ...
                    : {
                          ...
                          headers: {
                              Authorization:
                                  "Bearer sk-rebktjhdywuqfmulddzhdygglyrkeengnhlshvejdveeuwdw",
                              "Content-Type": "application/json",
                          },
                          body: JSON.stringify({
    ...
    saveConversations() {
        try {
            ...
            const conversationsToSave = JSON.parse(
            ...
            if (typeof chrome !== "undefined" && chrome.storage) {
                chrome.storage.local.set(
                    ...
                    () => {
                        ...
                        const dataSize =
                            JSON.stringify(conversationsToSave).length;
                        console.log(
                            `Saved ${this.conversations.length} conversations to chrome.storage (${dataSize} bytes)`,
                        );
                    },
                );
            } else {
                ...
                console.log(
                    `Saved ${this.conversations.length} conversations to localStorage (${dataSize} bytes)`,
                );
            }
        } catch (err) {
            ...
            if (err.name === "QuotaExceededError") {
                console.warn(
                    "Storage quota exceeded. Try clearing some history.",
                );
            }
        }
    }
            ...
            message.attachments.forEach((attachment) => {
                ...
                if (attachment.type === "pdf") {
                    ...
                    attachmentElement.appendChild(
                        document.createTextNode(attachment.name),
                    );
                }
    ...
    renderConversations(container) {
        ...
        if (!this.conversations || this.conversations.length === 0) {
            ...
            const noConversationsMsg =
                document.getElementById("noConversationsMsg");
        ...
        this.conversations.forEach((conversation) => {
            ...
            // 计算消息数量，用于显示会话信息
            const messageCount = conversation.messages
            ...
            deleteBtn.className =
                "btn btn-sm btn-outline-danger delete-conversation-btn";
            ...
            item.addEventListener("click", (e) => {
                console.log(
                    `Clicked on conversation ${conversationId} with ${messageCount} messages`,
                );
                // 防止事件冒泡，确保只处理一次点击事件
                e.preventDefault();
                e.stopPropagation();
